...
class App extends Manage
{
    ...
    public function register(): array
    {
        ...
        if (!$username || !$password) {
            throw new JSONException("请填写用户名和密码");
        }
        if (!$captcha) {
            throw new JSONException("请输入图形验证码");
        }
        if (!$cookie || !is_array($cookie) || empty($cookie)) {
            throw new JSONException("验证码已过期，请点击验证码图片刷新后重试");
        }
        $register = $this->app->register((string)$username, (string)$password, (string)$captcha, (array)$cookie);
        setConfig([
            "app_id" => $register["id"],
