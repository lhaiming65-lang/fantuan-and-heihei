                            ...
                            complete: (form, dom) => {
                                ...
                                function _register_captcha(loader = false) {
                                    util.post({
                                        ...
                                        error: (res) => {
                                            cookie = undefined;
                                            message.error(res?.msg || i18n("获取验证码失败，请检查网络连接后重试"));
                                        },
                                        fail: () => {
                                            cookie = undefined;
                                            message.error(i18n("获取验证码失败，请检查网络连接后重试"));
                                        }
                                    });
                                }
                      ...
                      <button type="button" class="btn btn-sm btn-light-success btn-register">
                        <i class="fa-duotone fa-regular fa-user-plus"></i> ${i18n('确认注册')}
                                ...
                                $('.btn-register').click(() => {
                                    ...
                                    const captchaVal = $("#register-captcha").val()?.trim();
                                    if (!username || !password) {
                                        message.error(i18n("请填写用户名和密码"));
                                        return;
                                    }
                                    if (!captchaVal) {
                                        message.error(i18n("请输入图形验证码"));
                                        return;
                                    }
                                    if (!cookie) {
                                        message.error(i18n("验证码已过期，请点击验证码图片刷新后重试"));
                                        $imageCode.click();
                                        return;
                                    }
